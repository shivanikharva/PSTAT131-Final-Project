...
Random forest #40 with all 14 predictors, 657 trees, and a minimal node size of 5 performed the best with an RMSE of 43.79638!  

## Fitting to Training Data  

Now, we will take that best model from the tuned random forest and fit it to the training data. This will train that random forest one more time on the entire training data set.  
...
# Save
write_rds(rf_final_fit, file = "data/rf_final/rf_final_train.rds")
```

```{r}
# Reading in the random forest fit on the training data
rf_final_fit_train <- read_rds(file = "data/rf_final/rf_final_train.rds")

# Collecting the RMSE of the random forest fit on the training data
rf_final_fit_train %>% 
  collect_metrics() %>% 
  slice(1)
```

The chosen random forest model had an RMSE of 43.95341 on the training data. This is quite close to the RMSE the model had on the folds.  
...
Our random forest actually performed slightly better on the testing set than on the training set and folds with an RMSE of 42.42378!  
