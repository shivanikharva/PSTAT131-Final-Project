...
### Ridge Regression   
```{r}
ridge_lasso_recipe <- recipe(capture_rate ~ attack + defense + hp + sp_attack + sp_defense + speed + type1 + generation + is_legendary + has_sturdy + has_swift_swim + has_keen_eye + has_chlorophyll + has_levitate, data = pokemon_train) %>%
  step_dummy(all_nominal_predictors()) %>% 
...
```{r}
best_lasso_penalty <- select_best(tune_res2, metric = "rsq")
...
poly_rec <- recipe(capture_rate ~ attack + defense + hp + sp_attack + sp_defense + speed + type1 + generation + is_legendary + has_sturdy + has_swift_swim + has_keen_eye + has_chlorophyll + has_levitate, data = pokemon_train) %>%
  step_dummy(all_nominal_predictors()) %>%
