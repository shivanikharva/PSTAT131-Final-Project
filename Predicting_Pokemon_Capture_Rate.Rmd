...
<center>![](https://media.tenor.com/8vuqpD3Ir-IAAAAC/catching-pokemon.gif)</center>  
...
<center>![Pokemon in battle!](https://static.wikia.nocookie.net/essentialsdocs/images/7/70/Battle.png/revision/latest?cb=20220523172438)</center>  
...
<center>![Pokemon stats screen.](https://78.media.tumblr.com/76016a9930e876e104b528190af0e125/tumblr_oxbo3x64Ge1w5miaho1_1280.png)</center>  
...
<center>![Left: Minior with its shell (Meteorite). Right: Minior without its shell (Core).](https://i1.wp.com/hallowpeakgaming.com/wp-content/uploads/2016/12/minior.jpg?fit=1024%2C486&ssl=1)</center>  
...
<center>![](https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/2aaacf79-3286-4339-9843-f60dfc484cc3/dety9on-74a91eb7-7f6a-43e7-a87f-67bcb6a4796b.gif?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..rFMKqBaSDLqE8knqyrMLA-DOqL7KOLTl7iawmzojIyI)</center>  
...
<center>![](https://media.tenor.com/tEGO5Q-EQIcAAAAC/pokemon.gif)</center>  
...
ggplot(pokemon_data, aes(capture_rate)) +
  geom_bar(fill='red3') +
  labs(
    title = "Distribution of Capture Rate"
  )
```
...
```{r}
# Boxplot of generation and legendary status vs. capture rate
pokemon_data %>% 
  ggplot(aes(x = capture_rate, y = reorder(is_legendary, capture_rate), fill = generation)) +
  geom_boxplot() +
  labs(
    title = "Box Plot of Generation and Legendary vs. Capture Rate",
    y = "Legendary Status", 
    x = "Capture Rate") +
  scale_fill_manual(values = c("red1", "blue1", "red3", "blue3", "darkred", "blue4", "#660000"))
```
...
pokemon_corrplot <- corrplot(pokemon_cor, method = "circle", addCoef.col = 1, number.cex = 0.7)
```
...
```{r, message = FALSE, warning=FALSE}
pokemon_data %>% 
  ggplot(aes(x=base_total, y=capture_rate)) + 
  geom_jitter(width = 0.5, size = 1) +
  geom_smooth(method = "lm", se =F, col="darkred") +
  labs(title = "Capture Rate vs. Base Total")
```

Although the relationship is not very strong, it is apparent that there is a moderately negative relationship between `capture_rate` and `base_total`. That is, a higher value of total battle statistics slightly relates to a lower capture rate. This makes contextual sense because a Pokemon with greater battle abilities would be more valuable (and harder to capture because of its greater value). So, it seems as if it would prove more meaningful to use `base_total` rather than the six battle statistics in exploring the relationship between the predictors and the outcome, `capture_rate`.
...
<center>![](https://pbs.twimg.com/media/EYoS4TmXgAIfLRD.jpg)</center>  
...
all_models <- data.frame(Model = c("Linear Regression", "Ridge Regression", "Lasso Regression", "Polynomial Regression", "K Nearest Neighbors", "Elastic Net", "Random Forest", "Boosted Trees"),
                         RMSE = c(lm_rmse$mean, ridge_rmse$mean, lasso_rmse$mean, poly_rmse$mean, knn_rmse$mean, elastic_rmse$mean, rf_rmse$mean, boosted_rmse$mean))

# Creating a barplot of the RMSE values
ggplot(all_models, aes(x=Model, y=RMSE)) +
  geom_bar(stat = "identity", aes(fill = Model)) +
  scale_fill_manual(values = c("blue1", "red1", "blue2", "red2", "blue3", "red3", "blue4", "red4")) +
...
<center>![](https://media4.giphy.com/media/6nWhy3ulBL7GSCvKw6/giphy.gif)</center>  
...
<center>![](https://www.ungeek.ph/wp-content/uploads/2016/11/ash_sparkiling-eyes.gif)</center>  
...
pokemon_tibble %>% 
  ggplot(aes(x = .pred, y = capture_rate)) +
...
<center>![](https://i.pinimg.com/originals/8f/23/e4/8f23e48ae48e3a07b342a718249e0080.gif)</center>  

# Sources

This data was taken from the Kaggle data set, "[Pokemon Stats Dataset](https://www.kaggle.com/datasets/hasanarcas/pokemon-stats-dataset)," and it was scraped from the [official Pokemon Database](https://pokemondb.net/pokedex/all) by user Hasan Arcas.

The information about Minior was found on [Bulbapedia](https://bulbapedia.bulbagarden.net/wiki/Minior_(Pok%C3%A9mon)), a community Pokemon encyclopedia.
